...
1\) Launch `deploy/src/utils/setBlockReward.js.` The `0xBLOCK_REWARD` is located in the  poa-network-consensus-contracts/contracts.json file.
...
3\) Replace the updated `spec.json` on all the other network nodes \(bootnode, explorer, validators\). Also update the `spec.json` on the rpc bootnode launched earlier and reboot.
